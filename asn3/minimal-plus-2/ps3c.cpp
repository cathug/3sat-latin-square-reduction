...
{
    propertyA(order, subset);

    // make all mapped values negative
    for (int i = 0; i < subset.size(); i++)
    {
        for (int j = 0; j < subset[i].size(); j++)
            subset[i][j].mappedVal = -(subset[i][j].mappedVal);
    }
}
...
void propertyE(const unsigned int &order,
               cnf &subset,
               const map<string, unsigned long> &Map)
{
    propertyC(order, subset, Map);
//    cout << subset.size() << endl;

    // make all mapped values positive
    for (int i = 0; i < subset.size(); i++)
    {
        for (int j = 0; j < subset[i].size(); j++)
            subset[i][j].mappedVal = abs(subset[i][j].mappedVal);
    }
...
void outputData(const char* fileOut,
                ...
                const cnf &subset5)
{
    long numClause = (unit_clause.size() + subset1.size() +
                      subset2.size() + subset3.size() +
                      subset4.size() + subset5.size());
//    cout << numClause << endl;
    ofstream output;
//    output.open("q_10_02c.cnf");
    output.open(fileOut);
    if (!output)
    {
...
//    cout << *order << endl;
    clause unit_clause;
    cnf subsetA, subsetB, subsetC, subsetD, subsetE;
    map<string, unsigned long> Map;
    ...
    propertyE(*order, subsetE, Map);

    // output data
    outputData(argv[2], *order,
               unit_clause, subsetA,
               subsetC, subsetD,
               subsetB, subsetE);
