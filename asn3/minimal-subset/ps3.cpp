...
void outputData(const char* fileOut,
                ...
                const cnf &subset3)
{
    int numClause = unit_clause.size() + subset1.size() + subset2.size() + subset3.size();
    ofstream output;
    output.open(fileOut);
    if (!output)
    {
    ...
    // output clauses matching minimal subsets
...
//    cout << *order << endl;
    ...
    // output data
    outputData(argv[2], *order, unit_clause, subsetA, subsetC, subsetD);
